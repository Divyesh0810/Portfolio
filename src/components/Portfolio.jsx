          ...
          {port.map(({ id, src, title, href, description, technologies }) => (
            ...
            >
              <div className="flex flex-col items-center p-4">
                <img src={src} alt="" className="rounded-md w-full h-48 object-cover" />
                <h2 className="text-lg font-semibold mt-4">{title}</h2>
              </div>
              <p className="text-center px-4 py-2 flex-grow">{description}</p>
              <div className="flex flex-wrap justify-center px-4">
                {technologies.map((tech, index) => (
                  <span key={index} className="bg-gray-700 text-white px-2 py-1 m-1 rounded-md text-sm">{tech}</span>
                ))}
              </div>
              <div className="flex justify-center mt-4 mb-4">
                <button className="px-6 py-3 bg-gray-700 rounded-md hover:bg-gray-600 transition-colors">
                  View In Github
                </button>
              </div>
            </div>
